...
def plot_stock_data(fig, df, ticker, relative_strength=None, sigma=0.005, min_change=0.002, window=3, min_duration=2):
    ...
    # Add Relative Strength information if provided
    if relative_strength is not None:
        try:
            rs_value = float(relative_strength.split(': ')[1])  # Extract the numeric value
            if rs_value > 0:
                color = 'lime'  # Bright green for strong positive values
            elif rs_value < 0:
                color = 'red'  # Red for negative values
            else:
                color = 'yellow'  # Yellow for neutral (zero) value
        except ValueError:
            # If conversion to float fails, it's probably 'N/A'
            color = 'white'

        ax1.text(0.02, 0.98, relative_strength, transform=ax1.transAxes,
                 verticalalignment='bottom', horizontalalignment='center',
                 fontsize=12, color=color, weight='bold',
                 bbox=dict(facecolor='black', edgecolor=color, alpha=0.7, pad=5))
